...
const WORK = [
  { tag: "Intake",     n: "01", t: "Voice agent",        d: "Answers every call, around the clock. Real-time slots and pricing, books on the spot, texts the quote — even at 11pm." },
  { tag: "Operations", n: "02", t: "Auto-dispatch",      d: "The right crew, matched by skill, location and availability the moment a job lands." },
  { tag: "Schedule",   n: "03", t: "Live calendar",      d: "Every job, every crew, every day — one continuous view that fills itself." },
  { tag: "Supplies",   n: "04", t: "Supplies & kit",     d: "A photo catalog crews actually recognise. They request refills and equipment; you approve in a tap." },
  { tag: "Logistics",  n: "05", t: "Tracked to the door",d: "Crews set their own delivery address; tracking and out-for-delivery alerts run themselves." },
  { tag: "Revenue",    n: "06", t: "Auto-payments",      d: "Cards charged the moment a job is marked complete — no invoicing, no chasing." },
];

const INDEX = [
  ["24/7 voice agent",          "Intake"],
  ["Instant quote & booking",   "Intake"],
  ["Quote sent by SMS",         "Intake"],
  ["Simultaneous calls",        "Intake"],
  ["Smart scheduling",          "Schedule"],
  ["Crew matching",             "Operations"],
  ["Staff mobile app",          "Operations"],
  ["Photo supplies catalog",    "Supplies"],
  ["Equipment requests",        "Supplies"],
  ["Request approval flow",     "Supplies"],
  ["Preferred shipping address","Logistics"],
  ["Automated tracking",        "Logistics"],
  ["Delivery alerts",           "Logistics"],
  ["White-label store",         "Logistics"],
  ["Auto-charge on completion", "Revenue"],
  ["Live analytics",            "Visibility"],
];

const SERVICES = [
  "24/7 voice agent", "Booking & dispatch", "Live calendar", "Supplies & equipment",
  "Logistics & tracking", "Auto-payments", "Staff mobile app", "Analytics",
];
      ...
      <div ref={contentRef} style={contentStyle}>
        ...
        <Section id="top" tag="header" className="hero">
          <div className="hero-main">
            <Reveal>
              <p className="eyebrow">AI ops agent — for service businesses</p>
            </Reveal>
            <Reveal delay={80}>
              <h1 className="mega">
                Operations,<br />
            ...
            <Reveal delay={180}>
              <p className="lede">
                neatr.ai is the AI ops agent that runs a service business end to end.
                It answers every call around the clock, books and dispatches the job,
                keeps crews stocked and shipped, and collects the payment — so the
                operator stops running the day and the work runs itself.
              </p>
            </Reveal>
            <Reveal delay={260}>
              <a className="herocta" href="#access"
                onClick={(e) => { e.preventDefault(); goTo("access"); }}>
          ...
          <Reveal delay={300}>
            <div className="herometa">
              <span className="herometa-tags">
                <b>Voice agent</b><i /><b>Dispatch</b><i /><b>Supplies &amp; logistics</b>
              </span>
        ...
        <Section id="live">
          <Reveal><div className="lbl"><span>Live</span><span>agent</span></div></Reveal>
          <div className="livegrid">
            <Reveal delay={60}>
              <h2 className="livehead">The call,<br /><span className="ital">answered.</span></h2>
              <p className="livep">
                11:04pm. A booking comes in by phone — neatr&apos;s voice agent quotes it,
                books it and texts the confirmation. No one picks up.
              </p>
            </Reveal>
        ...
        {/* SELECTED WORK */}
        <Section id="work">
          <Reveal><div className="lbl"><span>What it runs</span><span>06</span></div></Reveal>
        ...
        <Section id="studio" className="studio">
          <div className="studiogrid">
            <div>
              ...
              <Reveal delay={80}>
                <p className="bigbody">
                  neatr.ai is the AI ops agent for service
                  businesses — <span className="ital">cleaning, maintenance, field service and beyond.</span>
                  &nbsp;It runs the operation, not just the calendar: answering the
                  phone around the clock, booking and dispatching every job, keeping
                  crews stocked and shipped, and collecting payment — replacing the
                  missed calls, the spreadsheets and the supply-run group chats.
                </p>
              </Reveal>
              <Reveal delay={140}>
                <p className="meta">
                  Operator-first, in early access. Built with the crews who run on it.
                </p>
              </Reveal>
            </div>
            <div>
        ...
        <Section id="access" tag="footer" className="foot">
          ...
          <Reveal delay={180}>
            <div className="footrow">
              ...
              <span className="footnote">© 2026 neatr.ai — Operations, automated.</span>
            </div>
          </Reveal>
        </Section>
      </div>
...
const STEPS = [
  { k: "Call answered", s: "Inbound · 11:04pm · voice agent" },
  { k: "Quoted live",   s: "Deep clean · 2 bed · $185" },
  { k: "Booked",        s: "Thu 10:00 — slot reserved" },
  { k: "Dispatched",    s: "Maria L. · 0.8 mi · notified" },
  { k: "Texted",        s: "Confirmation + invoice by SMS" },
];
  ...
  return (
    <div className="bookcard">
      <div className="bookhead">
        ...
        <span className="bmono">neatr.ai · voice agent</span>
        <span className="blive"><span className="blivedot" />on call</span>
      </div>
      <div className="bookbody">
        <div className="bookticket">
          <span className="bmono" style={{ color: C.ink3 }}>Inbound call</span>
          <h3 className="booksvc">Deep clean<span className="ital"> — 2 bed</span></h3>
          <p className="bookcust">Sarah M. · 11:04pm</p>
...
const SETUP_STEPS = [
  { key: "signup", label: "Sign up", time: "0:22", secs: 22, copy: "An email and one code — a new business lands in its own live dashboard. Twenty-two seconds, unedited.", src: "https://cdn.neatr.ai/videos/01-tenant-signup.mp4", poster: "https://cdn.neatr.ai/videos/poster-01.jpg" },
  { key: "setup", label: "Set up", time: "1:45", secs: 105, copy: "Categories, spaces, services, pricing tiers, and branding — the whole catalog configured live, saving as you type.", src: "https://cdn.neatr.ai/videos/02-operations-setup.mp4", poster: "https://cdn.neatr.ai/videos/poster-02.jpg" },
  { key: "booking", label: "Customer books", time: "1:07", secs: 67, copy: "A guest checks their ZIP, picks an arrival window, pays — and walks away with a confirmed booking. No account needed.", src: "https://cdn.neatr.ai/videos/03-customer-booking.mp4", poster: "https://cdn.neatr.ai/videos/poster-03.jpg" },
];
...
function SetupFlow() {
  ...
  return (
    <div className="setupwrap">
      ...
      <p className="setupcopy">{step.copy}</p>
    </div>
  );
}

function Marquee({ reverse = false }: { reverse?: boolean }) {
  const items = ["24/7 voice agent", "Instant quotes", "Auto-dispatch", "Live calendar", "Supplies & kit", "Package tracking", "Auto-payments"];
...
.herocta{align-self:flex-start; display:inline-flex; align-items:center; gap:8px; margin-top:32px;
  background:${C.ink}; color:${C.bg}; border-radius:100px; padding:15px 28px; font-size:15px;
  font-weight:600; text-decoration:none; font-family:${SANS}; transition:opacity .2s, transform .2s;}
.herocta:hover{opacity:0.86; transform:translateY(-1px);}
.herocta span{transition:transform .2s;}
.herocta:hover span{transform:translateX(3px);}
...
.setupwrap{display:flex; flex-direction:column; gap:clamp(28px,3.4vw,40px);}
.setupcard{border:1px solid ${C.line}; background:rgba(255,255,255,0.86); backdrop-filter:blur(8px);
  border-radius:16px; overflow:hidden; box-shadow:0 30px 80px rgba(11,11,10,0.1);}
.setuphead{display:flex; align-items:center; gap:8px; padding:12px 18px; border-bottom:1px solid ${C.line};}
.setupimgwrap{position:relative; aspect-ratio:16/8; background:#FAFAF8; overflow:hidden;}
.setupimg{position:absolute; inset:0; width:100%; height:100%; object-fit:cover; object-position:top;
  animation:setupin 0.5s cubic-bezier(0.16,1,0.3,1);}
@keyframes setupin{from{opacity:0; transform:translateX(28px)} to{opacity:1; transform:none}}
.setuptabs{display:flex; flex-wrap:wrap; gap:clamp(16px,2.4vw,32px);}
.setuptab{flex:1; min-width:120px; text-align:left; background:none; border:none; cursor:pointer; padding:0;}
.setuptabnum{font-size:11px; color:${C.ink3}; margin-right:8px;}
.setuptablabel{font-family:${SERIF}; font-style:italic; font-size:16px; color:${C.ink3}; transition:color .3s;}
.setuptab.on .setuptablabel{color:${C.ink}; font-weight:600;}
.setuptabbar{display:block; height:2px; background:${C.line}; margin-top:10px; border-radius:100px; overflow:hidden;}
.setuptabfill{display:block; height:100%; background:${C.ink}; width:0%; animation:setupfill 4.2s linear forwards;}
@keyframes setupfill{to{width:100%}}
.setupcopy{font-size:15px; color:${C.ink2}; line-height:1.6; max-width:560px; margin:0 auto; text-align:center;}
...
@media(prefers-reduced-motion: reduce){
  ...
  .setupimg{animation:none !important;}
  .setuptabfill{animation:none !important; width:100%;}
}
`;
