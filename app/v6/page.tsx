  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        ...
        const alpha = 0.5 + Math.random() * 0.1;
        ctx.beginPath();
...
function HeroAppMock() {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setPhase(-1), 100); // trigger re-mount cycle
  ...
  const statusColor = ["#555","#22C55E","#3B82F6","#F59E0B","#8B5CF6","#EC4899"];
  ...
  return (
    <div style={{ position: "relative", height: 490, userSelect: "none" }}>
      ...
      <div style={{
        ...
        background: "#111215", borderRadius: 14,
        border: "1px solid rgba(255,255,255,0.1)",
        boxShadow: "0 32px 80px rgba(0,0,0,0.7)",
        overflow: "hidden", display: "flex", flexDirection: "column",
      }}>
        {/* top bar */}
        <div style={{ height: 36, background: "#0D0F11", borderBottom: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 14px", flexShrink: 0 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", flex: 1, overflow: "hidden" }}>
          ...
          <div style={{ borderRight: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1, borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              {[
                ...
                <div key={label} style={{ padding: "10px 12px", background: "#0E1012" }}>
                  <div style={{ fontSize: 9, color: "rgba(255,255,255,0.35)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>{label}</div>
                  <div style={{ fontSize: 18, fontWeight: 700, color: glow ? color : "#fff", letterSpacing: "-0.03em", transition: "color 0.4s", textShadow: glow ? `0 0 12px ${color}88` : "none" }}>{value}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ padding: "10px 12px", flex: 1 }}>
              <div style={{ fontSize: 9, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8 }}>Schedule · May</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 3, marginBottom: 6 }}>
                {DAYS.map(d => <div key={d} style={{ fontSize: 7, color: "rgba(255,255,255,0.25)", textAlign: "center" }}>{d}</div>)}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 3 }}>
                {HOURS.map((h, i) => (
                  <div key={h} style={{
                    height: 28, borderRadius: 4,
                    background: calSlots[i] ? "rgba(34,197,94,0.25)" : "rgba(255,255,255,0.05)",
                    border: calSlots[i] ? "1px solid rgba(34,197,94,0.5)" : "1px solid transparent",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: 8, color: calSlots[i] ? "#22C55E" : "rgba(255,255,255,0.2)",
                    transition: "all 0.4s",
                    boxShadow: calSlots[i] ? "0 0 8px rgba(34,197,94,0.2)" : "none",
                  }}>
                    {calSlots[i] ? "●" : h}
                  </div>
                ))}
              ...
              <div style={{ marginTop: 12, borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 10 }}>
                <div style={{ fontSize: 9, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8 }}>Providers</div>
                {[
                  ...
                  <div key={name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <div style={{ width: 20, height: 20, borderRadius: "50%", background: "rgba(255,255,255,0.08)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, color: "#fff", fontWeight: 700 }}>{name[0]}</div>
          ...
          <div style={{ display: "flex", flexDirection: "column", padding: "10px 0" }}>
            <div style={{ fontSize: 9, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.06em", padding: "0 12px", marginBottom: 8 }}>Live activity</div>
            <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column", gap: 1 }}>
              {log.length === 0 && (
                <div style={{ padding: "16px 12px", textAlign: "center" }}>
                  <div style={{ fontSize: 10, color: "rgba(255,255,255,0.2)", lineHeight: 1.6 }}>Waiting for<br />next booking…</div>
                  <div style={{ marginTop: 8, display: "flex", justifyContent: "center", gap: 3 }}>
                    {[0,1,2].map(i => (
                      <span key={i} style={{ width: 4, height: 4, borderRadius: "50%", background: "rgba(255,255,255,0.15)", animation: `pulse 1.2s ease-in-out ${i*0.2}s infinite` }} />
                    ))}
                  </div>
                </div>
              )}
              {log.map((entry, i) => (
                <div key={entry.id} style={{
                  ...
                  transition: "all 0.4s",
                  animation: i === 0 ? "slideIn 0.4s cubic-bezier(0.16,1,0.3,1)" : "none",
                }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 2 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
                      <span style={{ fontSize: 11 }}>{entry.icon}</span>
                      <span style={{ fontSize: 11, fontWeight: 600, color: i === 0 ? entry.color : "rgba(255,255,255,0.55)" }}>{entry.text}</span>
                    </div>
                    <span style={{ fontSize: 9, color: "rgba(255,255,255,0.2)", fontFamily: "monospace" }}>{entry.ts}</span>
                  </div>
                  <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", paddingLeft: 16 }}>{entry.sub}</div>
                </div>
              ))}
            </div>

            {/* pipeline status bar */}
            <div style={{ padding: "10px 12px", borderTop: "1px solid rgba(255,255,255,0.06)", marginTop: "auto" }}>
              <div style={{ fontSize: 9, color: "rgba(255,255,255,0.3)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8 }}>Pipeline</div>
              <div style={{ display: "flex", gap: 3 }}>
                {["Received","Paid","Scheduled","Matched","Sent"].map((s, i) => (
                  <div key={s} style={{ flex: 1 }}>
                    <div style={{
                      height: 3, borderRadius: 2,
                      background: phase > i ? statusColor[i+1] : "rgba(255,255,255,0.08)",
                      transition: "background 0.5s",
                      boxShadow: phase > i ? `0 0 6px ${statusColor[i+1]}66` : "none",
                    }} />
                    <div style={{ fontSize: 8, color: phase > i ? "rgba(255,255,255,0.5)" : "rgba(255,255,255,0.2)", marginTop: 4, textAlign: "center" }}>{s}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* pulse dot overlay on new event */}
      {phase === 1 && (
        <div style={{ position: "absolute", top: 12, right: 12, width: 8, height: 8, borderRadius: "50%", background: "#22C55E", boxShadow: "0 0 0 4px rgba(34,197,94,0.3)" }} />
      )}
...
/* ─── Booking flow demo (iframe embed) ───────────────────────────────────── */
    ...
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: "#0A0A0A", color: "#fff", overflowX: "hidden" }}>
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 32 }}>
          <div style={{ display: "flex", gap: 28, fontSize: 13, color: "rgba(255,255,255,0.6)" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <a
              href="http://localhost:4100/auth/login"
              target="_blank"
              rel="noreferrer"
              style={{
                padding: "6px 14px",
          ...
          <div>
            ...
            <Reveal delay={60}>
              ...
              >
                ...
                system,
                <br />
                <span style={{ color: "#22C55E" }}>automated.</span>
              </h1>
            </Reveal>
            ...
            <Reveal delay={240}>
              <div style={{ display: "flex", gap: 24, marginTop: 40 }}>
                {[
                  ...
                  { n: "50K+", l: "bookings to process" },
                  { n: "4.9★", l: "beta rating" },
                ].map(({ n, l }) => (
                  <div key={n}>
                    <div style={{ fontSize: 22, fontWeight: 700, color: "#fff", letterSpacing: "-0.03em" }}>{n}</div>
      ...
      >
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <Reveal>
            <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", color: "#999", marginBottom: 24, textTransform: "uppercase" }}>
              The problem
            </p>
          </Reveal>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "start" }}>
            ...
            <div>
              <Reveal delay={100}>
                <p style={{ fontSize: 17, lineHeight: 1.7, color: "#555", margin: "0 0 32px" }}>
                  Most home service businesses run on a patchwork of Google Sheets, text messages,
                  and manual bank transfers. Every booking takes 20 minutes of back-and-forth.
                  Every payment is chased manually.
                </p>
              </Reveal>
                ...
                {[
                  ...
                  <Reveal key={t} delay={140 + i * 40}>
                    <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                      ...
                      <span style={{ fontSize: 15, color: "#444", lineHeight: 1.55 }}>{t}</span>
                    </div>
                  </Reveal>
                ))}
              </div>
      ...
      >
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 80, alignItems: "center" }}>
            <div>
              <Reveal>
                <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", color: "#22C55E", marginBottom: 24, textTransform: "uppercase" }}>
                  Fix the loop
                </p>
              </Reveal>
              <Reveal delay={60}>
                ...
                >
                  ...
                  Everything else
                  <br />
                  <span style={{ color: "#22C55E" }}>runs itself.</span>
                </h2>
              </Reveal>
              <Reveal delay={100}>
                <p style={{ fontSize: 16, lineHeight: 1.7, color: "rgba(255,255,255,0.5)", margin: "0 0 36px" }}>
                  Customers book from any device, pick their service, and confirm in seconds. Your calendar
                  fills automatically. The right team member gets dispatched. You see everything in real time.
                </p>
              </Reveal>
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {[
            ...
            <Reveal from="right" delay={160}>
              ...
              <p style={{ textAlign: "center", fontSize: 12, color: "rgba(255,255,255,0.3)", marginTop: 12 }}>
                ↑ This is the real booking flow — try it
              </p>
            </Reveal>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 80, alignItems: "center" }}>
            ...
            <div>
              ...
              <Reveal delay={100}>
                <p style={{ fontSize: 16, lineHeight: 1.7, color: "#555", margin: "0 0 32px" }}>
                  Your calendar auto-populates as bookings come in. AI scheduling routes jobs to
                  the right team member based on availability, location, and skills — no manual
                ...
                {[
                  {
                    ...
                    body: "See every booking, every provider, every day — at a glance.",
                  },
                  {
                    icon: "🤖",
                    title: "AI auto-assign",
                    body: "Define rules once. The system handles dispatch automatically.",
                  },
                  {
                    icon: "🔔",
                    title: "Provider notifications",
                    body: "Team gets SMS/email the moment they're assigned to a job.",
                  },
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1.5fr", gap: 80, alignItems: "center" }}>
            <div>
              ...
              <Reveal delay={100}>
                <p style={{ fontSize: 16, lineHeight: 1.7, color: "rgba(255,255,255,0.5)", margin: "0 0 36px" }}>
                  Revenue trends, booking status breakdown, team performance, and customer
                  retention — all in one dashboard. Know your business at a glance.
                </p>
              </Reveal>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                {[
                  ...
                  <Reveal key={label} delay={140 + i * 40}>
                    ...
                    >
                      <div style={{ fontSize: 11, color, fontWeight: 700, letterSpacing: "0.05em", marginBottom: 8, textTransform: "uppercase" }}>
                        {label}
                      </div>
                      <div style={{ fontSize: 24, fontWeight: 700, color: "#fff", letterSpacing: "-0.04em" }}>—</div>
                      <div style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", marginTop: 4 }}>Live from your account</div>
                    </div>
                  </Reveal>
                ))}
              </div>
      ...
      >
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <Reveal>
            <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", color: "#999", marginBottom: 16, textTransform: "uppercase" }}>
              How it works
            </p>
          </Reveal>
          <Reveal delay={40}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 40 }}>
            {[
              ...
              { n: "04", title: "Job done, payment collected", body: "Once complete, receipts are sent, reviews requested, and the next recurring booking is already queued.", tag: "Runs itself" },
              ...
              <Reveal key={n} delay={i * 80}>
                <div style={{ position: "relative" }}>
                  ...
                  <div style={{ paddingLeft: 20 }}>
                    ...
                    <p style={{ fontSize: 14, lineHeight: 1.65, color: "#666", margin: "0 0 16px" }}>{body}</p>
                    <span
                      style={{
      ...
      >
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <Reveal>
            <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.1em", color: "#22C55E", marginBottom: 16, textTransform: "uppercase" }}>
              Pricing
            </p>
          </Reveal>
          <Reveal delay={40}>
          ...
          <Reveal delay={80}>
            <p style={{ fontSize: 17, color: "rgba(255,255,255,0.45)", margin: "0 0 64px" }}>
              Lock in early access pricing before we launch.
            </p>
          </Reveal>
            ...
            {[
              {
                name: "Starter",
                price: "Free",
                sub: "For new businesses",
                features: ["Up to 20 bookings/month", "Online booking page", "Customer portal", "Basic analytics"],
                highlight: false,
              },
              {
                name: "Business",
                price: "$49",
                sub: "/mo · most popular",
                features: ["Unlimited bookings", "Smart team dispatch", "Automated payments", "Custom branding", "Priority support", "Advanced analytics"],
                highlight: true,
              },
              ...
              <Reveal key={name} delay={120 + i * 60}>
                <div
                  style={{
                    padding: "32px",
                    border: highlight ? "1px solid rgba(34,197,94,0.4)" : "1px solid rgba(255,255,255,0.1)",
                    borderRadius: 12,
                ...
                >
                  ...
                  <div style={{ fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.5)", marginBottom: 12 }}>{name}</div>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginBottom: 4 }}>
                    <span style={{ fontSize: 36, fontWeight: 700, letterSpacing: "-0.04em" }}>{price}</span>
                  </div>
                  <div style={{ fontSize: 13, color: "rgba(255,255,255,0.35)", marginBottom: 28 }}>{sub}</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 32 }}>
                    {features.map((f) => (
                      <div key={f} style={{ display: "flex", gap: 10, alignItems: "center" }}>
                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={highlight ? "#22C55E" : "rgba(255,255,255,0.4)"} strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>
          ...
          <Reveal delay={100}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, justifyContent: "center" }}>
              ...
              <div style={{ textAlign: "left" }}>
                <div style={{ fontSize: 14, fontWeight: 600 }}>Jamie Rodriguez</div>
                <div style={{ fontSize: 13, color: "#666" }}>Owner, Sparkle Clean NYC · Beta tester</div>
              </div>
            </div>
          </Reveal>
        </div>
      ...
      >
        ...
        <div style={{ maxWidth: 640, margin: "0 auto", textAlign: "center", position: "relative" }}>
          <Reveal>
            <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: "0.1em", color: "rgba(0,0,0,0.5)", marginBottom: 20, textTransform: "uppercase" }}>
          ...
          <Reveal delay={80}>
            <p style={{ fontSize: 17, color: "rgba(0,0,0,0.6)", margin: "0 0 40px", lineHeight: 1.65 }}>
              Join 500+ home service businesses on the waitlist. Early members get free access
